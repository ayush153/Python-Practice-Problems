# python practice problem 7
# You are given few sentences as a list (Python list of sentences). Take a query
# string as an input from the user. You have to pull out the sentences matching
# this query inputted by the user in decreasing order of relevance after converting
# every word in the query and the sentence to lowercase. Most relevant sentence is
# the one with the maximum number of matching words with the query.
...
if __name__ == "__main__":
    ...
    # print(scores)
    sortedSentScore = [
        sentScore for sentScore in sorted(zip(scores, sentences), reverse=True)
        if sentScore[0] != 0
    ]
